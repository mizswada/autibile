...
const downloadInvoice = async (invoiceId) => {
  try {
    ...
    // Build the PDF using the shared new-design renderer
    const pdf = await renderDocumentPdf(jsPDF, buildInvoiceDocOptions(invoice));
...
const printInvoice = (invoice) => {
  ...
  const printContent = buildDocumentHtml(buildInvoiceDocOptions(invoice));
...
const downloadReceipt = async (paymentId) => {
  try {
    ...
    // Build the PDF using the shared new-design renderer
    const pdf = await renderDocumentPdf(jsPDF, buildReceiptDocOptions(payment));
...
const printReceipt = (payment) => {
  ...
  const printContent = buildDocumentHtml(buildReceiptDocOptions(payment));
  openPrintWindow(printContent);
};
