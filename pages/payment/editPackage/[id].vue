  ...
  try {
    ...
    if (response.statusCode === 200) {
      ...
      form.value = {
        ...
        availableSessions: packageData.avail_session.toString(),
...
const updatePackage = async () => {
  ...
  try {
    const response = await $fetch(`/api/payment/updatePackage/${packageId}`, {
      method: 'PUT',
      body: {
    ...
    if (response.statusCode === 200) {
      ...
      showMessage(response.message || 'Failed to update package', 'error');
    }
  } catch (error) {
    console.error('Error updating package:', error);
    showMessage(error.data?.message || 'An error occurred while updating the package', 'error');
  } finally {
